   ...
   /* 检查：如果商品有规格，而post的数据没有规格，把商品的规格属性通过JSON传到前台 */
    ...
    else
    {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent))
        {
			...
			$result['cart_number'] = insert_cart_info_number();  //没有的函数会影响 在lib_insert.php内
        }
        else
        {
...
$_SESSION['direct_shopping']=1;
    ...
    $province=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id=". $consignee['province']);
    $city=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id=". $consignee['city']);
    $district=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id=". $consignee['district']);
...
/*------------------------//var_dump(unserialize_config($payment['pay_config'])); //支付接口appid信息	------------------------------ */

/*-------------------------//var_dump($pay_online);　//支付接口appid信息----------------------------- */
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
		...
		'zhekou'      => $_POST['zhekou'], //周五特惠折扣
	    'besttime'      => $_POST['besttime'], //送货时间
