...
if(!isset($user_id))
{
ecs_header('Location: http://www.gzqiaguo.com/wechat/oauth/wxch_oauth.php?oid=1');
}
else
{
//echo "已登陆";
...
if ($action == 'default')
{
	...
	if(!empty($wxid)){
		$weixinInfo = $GLOBALS['db']->getRow("SELECT nickname, headimgurl FROM site_weixin_user WHERE wxid = '$wxid'");
	...
	if($rank_points>$level_register_up||$rank_points==$level_register_up){
		...
		$tianxin="掌柜：否     <a href='http://www.weourgo.cn/mobile/goods.php?id=150'>点此链接购买成为掌柜</a>";
	}
...
$kuaidi=ecs_kuaidi_state($orders['order_sn']);
...
//czneng
	if($order_id)
	{
     ...
     $ordershippingname= $GLOBALS['db']->GetOne("SELECT  ship_note FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'");
	if ($ordershippingname)
		{
		$order['shipping_name']= $ordershippingname;
		}
	}
//czneng
...
//var_dump($payment_info['pay_button']);
...
elseif ($action == 'fenxiao1')
{
	...
	foreach($user_list['user_list'] as $key =>$value){
		...
		if($value['level']==1){
			...
			$wxid=$value['wxid'];
			$value['head_url']=$GLOBALS['db']->getOne("SELECT  headimgurl FROM site_weixin_user WHERE wxid = '$wxid'");
			$value['nickname']=$GLOBALS['db']->getOne("SELECT nickname FROM site_weixin_user WHERE wxid = '$wxid'");
			$new_arr[]=$value;
		}
	}
...
function ecs_kuaidi_state($order_sn)
{


 $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('kuaidi_state') . " where orderid = ".$order_sn."   ";
 $res = $GLOBALS['db']->getAll($sql);

 $arr = array();
    foreach ($res AS $idx => $row)
    {
        $arr[$idx]['orderid']          = $row['orderid'];
        $arr[$idx]['statetime']       = $row['statetime'];
		 $arr[$idx]['stateinfo']       = $row['stateinfo'];
  $arr[$idx]['com']       = $row['com'];
        $arr[$idx]['nu']     = $row['nu'];
    }
    return $arr;
